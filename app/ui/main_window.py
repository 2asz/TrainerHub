...
class CardDelegate(QStyledItemDelegate):
    ...
    def _render_bg(self, selected, running, hover):
        ...
        # 背景 + 边框
        if selected:
            bg, border, bw = QColor(31, 48, 72), QColor(C_ACCENT), 2
        elif running:
            bg, border, bw = QColor(20, 40, 30), QColor(C_RUNNING), 2
        else:
            bg = QColor(C_CARD_HOVER if hover else C_CARD)
            border = QColor(C_ACCENT if hover else C_BORDER)
            bw = 1
        p.setBrush(bg)
    ...
    def paint(self, painter, option, index):
        ...
        tag_top = info_top + 19
        for t in tags[:3]:
            painter.setBrush(self._tag_colors.get(t, QColor(170, 170, 180)))
            painter.setPen(Qt.NoPen)
            painter.drawEllipse(QPoint(x + 4, tag_top + 8), 3, 3)
            painter.setPen(QColor(160, 168, 178))
            painter.drawText(QRect(x + 11, tag_top, 100, 16),
                             Qt.AlignLeft | Qt.AlignVCenter, t)
            x += 11 + painter.fontMetrics().horizontalAdvance(t) + 13
        ...
        self._paint_btn(painter, game_btn, "▶ 启动游戏", btn_state("game"),
                        (61, 139, 253), (90, 160, 255), (44, 111, 214),
                        (255, 255, 255))
        # 修改器（有→绿，无→灰"添加"）
        if cnt:
            self._paint_btn(painter, trainer_btn, f"⚡ 修改器({cnt})",
                            btn_state("trainer"),
                            (31, 143, 77), (40, 172, 95), (23, 105, 59),
                            (255, 255, 255))
        else:
            self._paint_btn(painter, trainer_btn, "＋ 添加修改器",
                            btn_state("trainer"),
                            (52, 59, 70), (61, 70, 84), (38, 44, 54),
                            (180, 188, 198))
        ...
        if running:
            ...
            painter.setPen(Qt.NoPen)
            painter.setBrush(QColor(16, 90, 44, 225))
            painter.drawRoundedRect(chip, 11, 11)
            painter.setPen(QColor(120, 240, 170))
...
class SidebarDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if selected or hover:
            ...
            painter.fillPath(path, QColor("#1d2531" if selected else "#1b2029"))
        if selected:
            bar = QRect(rect.left(), rect.top() + 6, 3, rect.height() - 12)
            painter.fillRect(bar, QColor(C_ACCENT))
        ...
        painter.setPen(QColor(C_TEXT if (selected or hover) else C_TEXT_DIM))
        painter.drawText(name_rect, Qt.AlignLeft | Qt.AlignVCenter,
                         painter.fontMetrics().elidedText(
                             name, Qt.ElideRight, name_rect.width()))
        if count is not None:
            painter.setFont(QFont(option.font))
            painter.setPen(QColor("#7ea8f0" if selected else "#6b7686"))
...
class _SteamImportWorker(QThread):
    """后台线程：解析快捷方式 + 拉取 appinfo，逐条发回主线程。
            ...
            if folder.is_dir():
                for f in sorted(folder.iterdir()):
                    if self._stop.is_set():
                        break
                    if f.suffix.lower() not in (".url", ".lnk"):
                        continue
                    parsed = parse_shortcut(f)
                    if not parsed:
                        continue
                    total += 1
                    self.progress.emit(total, ok)
                    if "appid" in parsed:
                        try:
                            info = self._app_info.fetch(parsed["appid"],
                                                        cancel=self._stop)
                        except Exception:
                        ...
                        if info and info.get("name"):
                            ok += 1
                        self.found_steam.emit(parsed["appid"], Path(f.name).stem, info)
                    elif parsed.get("launch"):
                        ok += 1
                        self.found_lnk.emit(Path(f.name).stem, parsed["launch"])
        finally:
            # 无论正常结束还是异常/取消，都必须发送 finished，
            # 否则导入进度框永不关闭
...
class _UpdateCheckWorker(QThread):
    ...
    def run(self):
        ...
        for i, (gid, t) in enumerate(tasks):
            ...
            try:
                results = self._adapter.search(game["name"])
    ...
    def _best_match_page(game_name, results) -> str | None:
        ...
        归一化（小写去符号）后：互含=强匹配；否则相似度。低于阈值返回 None。"""
        def norm(s):
        ...
        for r in results or []:
            ...
            if a == b:
                ...
                score = 0.9            # 互含（次强：如 "AI LIMIT" ⊂ "AI LIMIT 无限机兵"）
            else:
                score = difflib.SequenceMatcher(None, a, b).ratio()
            if score > best_score:
                best_score, best_url = score, r.get("page_url")
...
class MainWindow(QMainWindow):
    def __init__(self, library: Library):
        super().__init__()
        self._library = library
        # 启动清理：磁盘上已被手动删除的修改器，移除库记录（否则一直显示）
        pruned = library.prune_missing_trainers()
        if pruned:
            audit.info(f"启动清理：{pruned} 个修改器文件不存在，已移除对应记录")
        # 启动清理：已卸载的游戏（Steam 清单消失/本地 exe 被删），移除库记录
        gone = _prune_missing_games(library)
        if gone:
            audit.info(f"启动清理：{len(gone)} 款游戏已卸载，移除记录：{'、'.join(gone)}")
        # 启动清理：cover_file 已无效（用户删过 data/covers/）清掉引用，
        # 否则封面补拉会误以为"有封面"而跳过，永远不重新生成
        stale_covers = _prune_missing_covers(library)
        if stale_covers:
            audit.info(f"启动清理：{stale_covers} 条封面引用已失效，等待重新生成")
    ...
    def _build_toolbar(self):
        ...
        brand = QLabel("🎮  Trainer Hub", self)
        brand.setStyleSheet("color: %s; font-size: 15px; font-weight: bold;"
                            "padding: 0 14px 0 6px;" % C_TEXT)
        tb.addWidget(brand)
        ...
        self._stats_label.setStyleSheet("color: %s; padding: 0 12px;" % C_TEXT_DIM)
    ...
    def _build_empty_page(self):
        ...
        self._empty_title.setStyleSheet(
            "color: %s; font-size: 20px; font-weight: bold;" % C_TEXT)
        self._empty_title.setAlignment(Qt.AlignCenter)
        self._empty_desc = QLabel("", page)
        self._empty_desc.setStyleSheet("color: %s; font-size: 13px;" % C_TEXT_DIM)
    ...
    def _style_sheet(self):
        ...
        QToolBar#mainToolbar {{ background: #14171d; border: none; padding: 5px;
                               border-bottom: 1px solid {C_BORDER}; spacing: 2px; }}
        /* 按钮点击反馈：:hover=悬停高亮，:pressed=按下时颜色加深 +
           内容下沉 1px（增加/减少上边距实现的"按压感"） */
        QToolButton {{ padding: 6px 11px; border-radius: 6px; color: {C_TEXT}; }}
        QToolButton:hover {{ background: #232a35; }}
        QToolButton:pressed {{ background: {C_ACCENT_DARK};
                              padding: 7px 11px 5px 11px; }}
        QToolBar::separator {{ background: {C_BORDER}; width: 1px; margin: 4px 8px; }}
        QLineEdit#searchBox {{ background: #1a1f28; border: 1px solid {C_BORDER};
                              border-radius: 14px; padding: 5px 12px;
                              selection-background-color: {C_ACCENT}; }}
        QLineEdit#searchBox:focus {{ border: 1px solid {C_ACCENT}; }}
        QListWidget#sidebar {{ background: {C_SIDEBAR}; border: none; padding-top: 6px;
                              font-size: 13px; }}
        ...
        QMenu {{ background: #1b1f27; border: 1px solid {C_BORDER}; padding: 5px;
                border-radius: 6px; }}
        QMenu::item {{ padding: 7px 24px; border-radius: 4px; color: {C_TEXT}; }}
        QMenu::item:selected {{ background: {C_ACCENT_DARK}; }}
        QMenu::separator {{ height: 1px; background: {C_BORDER}; margin: 4px 8px; }}
        QMessageBox {{ background: #171a21; }}
        QMessageBox QLabel {{ color: {C_TEXT}; }}
        QStatusBar {{ background: #14171d; border-top: 1px solid {C_BORDER};
                     color: {C_TEXT_DIM}; font-size: 12px; }}
        QStatusBar::item {{ border: none; }}
        QPushButton#emptyBtn {{
            background: #1f2630; border: 1px solid {C_BORDER}; border-radius: 8px;
            padding: 9px 18px; font-size: 13px; color: {C_TEXT};
        }}
        QPushButton#emptyBtn:hover {{ background: #2a3341; border-color: {C_ACCENT}; }}
        QPushButton#emptyBtn:pressed {{ background: #1d242f;
                                        border-color: {C_ACCENT_DARK};
                                        padding: 10px 18px 8px 18px; }}
        QWidget#emptyPage {{ background: {C_BG}; }}
        """
    ...
    def _schedule_cover_fetch(self):
        ...
        - 所有类型（本地/Steam/Epic）：先离线生成 exe 图标封面；找不到图标时
          用游戏名首字母兜底，保证每款游戏启动后都有图；
        ...
        try:
            ...
            for g in self._library.all_games():
                ...
                try:
                    ...
                    # 生成失败不能静默吞掉，否则用户会看到"无封面"却不知道原因
                    audit.warning(f"离线封面生成失败 {g.get('id')} {g.get('name')}: {e}")
                    cover = None
                if cover:
                    self._library.update_game(g["id"], cover_file=cover)
                    # 封面来源变了，清除加载器里可能存在的失败占位，让新封面立即被加载
                    self._covers.forget(g["id"])
                    self._model.cover_updated(g["id"])
                    changed = True
            if changed:
                self._mark_save()
        except Exception as e:
    ...
    def check_trainer_updates(self):
        ...
        if not any(t.get("downloaded") for _, t in self._library.all_trainers()):
            QMessageBox.information(self, "检查更新", "库中没有官网下载的修改器。")
            return
    ...
    def _upd_check_done(self, dlg, total, fails):
        ...
        if not items:
            extra = f"（{fails} 个网络失败已跳过）" if fails else ""
            QMessageBox.information(
                self, "检查更新",
                f"所有修改器均已最新（共检查 {total} 个）{extra}")
            return
        lines = "\n".join(
    ...
    def refresh_state(self):
        ...
        2. 立即重扫进程并重新判定"运行中/已停止"（force_poll 当场发信号，
           徽章马上熄灭/点亮）；
        3. 重建模型视图（分类/搜索过滤）并刷新侧边栏计数与状态栏。
        完成后在状态栏给一句反馈。"""
        n = self._proc_watch.prune_background_process_names()
        if n:
        ...
        self.statusBar().showMessage(
            f"已刷新：清理 {n} 个误关联进程名，运行状态已重新检测"
            if n else "已刷新：运行状态已重新检测", 3000)
