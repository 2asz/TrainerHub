...
def _exe_icon_pixmap(exe_path, size=128):
    """提取 exe 大尺寸图标：优先 SHDefExtractIconW（可取 128px 源，清晰），
    失败回退 QFileIconProvider（多为 32/48px 源）。返回空 QPixmap 表示失败。"""
    import ctypes
    ...
    try:
        ...
        pass
    # 回退：系统图标提供器（源通常 32/48px）
    from PySide6.QtCore import QFileInfo
    from PySide6.QtWidgets import QFileIconProvider
    return QFileIconProvider().icon(QFileInfo(str(exe_path))).pixmap(size, size)
...
def make_icon_cover(gid, exe_path, w=268, h=108) -> str | None:
    ...
    try:
        ...
        pm = _exe_icon_pixmap(exe_path, size=256)
        if pm.isNull():
            return None
